...
При отсутствии пользователя в базе данных он будет добавлен автоматически, однако
при добавлении несуществующего сегмента вернется ошибка, так как следует предварительно
добавить сегмент по соответствующему эндпоинту.
...
- **Ответы**:
  - `201 Created`Возвращает путь к файлу с логами в контейнере. Название: `(id пользователя_дата).csv`
  - `400 Bad Request`
  - `500 Internal Server Error`
...
1. **Логирование**
   ...
   Для решения данной задачи в базе данных создана отдельная таблица `logs`, которая представлена структурой `Log`:
...
2. **TTL**
    ...
    В `main` запускается отдельная горутина `WorkerTtl` и каждые 30 секунд проверяет, есть ли отношения, которые должны быть удалены на основе поля `ExpirationDate`.
    ...
    `WorkerTtl` вызывает функцию `DeleteExpiredSegments`, которая в свою очередь обращается к репозиторию для удаления устаревших записей. После успешного удаления, в таблицу `logs` добавляются соответствующие записи.
...
3. **Миграции**
    ...
    В задании написано, что не нужно писать миграции, но для удобства я реализовал автоматические GORM миграции при запуске приложения. Их можно
отключить, поставив в `config/default.yaml` поле `migrations как false`
.Файл с таблицами лежит в `pkg/storage/migrations`.
